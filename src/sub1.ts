...
function func1<T>(a: T, b: number, c: number = 3): void {
  console.log(a);
  console.log(b);
  console.log(c);
}

function func2({ a, b = '222' }: { a: string; b?: string }, c: number = 333): void {
...
// function signature annotation takes the priority than actual function setup
type Func3Type = ({ a, b }: { a: string; b?: string | number }, c?: number) => boolean;

const func3: Func3Type = ({ a, b = 'B DEFAULT' }: { a: string; b?: string | number }, c: number = 444): boolean => {
...
function func4(a: { a: string; b: string }) {
  console.log(a.a);
}
